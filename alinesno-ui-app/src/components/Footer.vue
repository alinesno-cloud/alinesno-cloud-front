<template>
	<view>
		<view class="footer_nav" :style="{'padding-bottom': buBottom + 'rpx'}">
			<navigator class="footer_nav_item" :class="activeIndex===index?'on':''" :url="index==0?firstLink[firstLinkIndexA]:item.path" open-type="reLaunch" hover-class="none" v-for="(item,index) in tabbar" :key="index">
				<image :src="activeIndex===index?item.selectIcon:item.icon"></image>
				<view class="footer_nav_text">{{index==0?firstNavName[firstNameIndexA]:item.name}}</view>
			</navigator>
		...
		<view v-if="systemType==2">
			...
			<view class="diy" :class="{up: is_quick}">
				<view class="nav">
					<view class="nav_link" @click="goPage('/pages/quality/work/checked')">
						<image src="/static/images/d1.png" mode=""></image>
						<view class="name">质量检查</view>
					</view>
					<view class="nav_link" @click="goPage('/pages/quality/work/accept')">
						<image src="/static/images/d2.png" mode=""></image>
						<view class="name">质量验收</view>
					</view>
					<view class="nav_link" @click="openSan">
						<image src="/static/images/d7.png" mode=""></image>
						<view class="name">扫一扫</view>
...
<script>
	...
	export default {
		...
		data(){
			return {
				tabbar: [
					...
					{
						"path": uni.getStorageSync('systemType')==1 ? "/pages/mine/mine" : '/pages/quality/mine/mine',
						"name": "我的",
				...
				firstNavName: ['隐患排查','危险作业','风险管控','安全验收','工序报验'],
				
				firstLink: [
					'/pages/index/indexYinhuan',
					'/pages/danger/index',
					'/pages/riskControl/riskControl/riskControl',
					'/pages/safetyCheck/safetyCheck/safetyCheck',
					'/pages/index/qualityIndex',
				],
		...
		created() {
			...
			this.isIPhoneX();
			// #endif
			if(this.firstNameIndex && this.firstLinkIndex){
				let name= {
			...
			if(defaultData && defaultData.index){
